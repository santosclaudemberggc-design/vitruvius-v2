...
try {
    ...
    if ($result.ok) {
        if ($result.result.count -eq 0) {
            Write-Host "AVISO: Nenhum elemento selecionado no Revit." -ForegroundColor Yellow
        } else {
            Write-Host "SUCESSO!" -ForegroundColor Green
            Write-Host "`nElementos selecionados ($($result.result.count)):" -ForegroundColor Green
